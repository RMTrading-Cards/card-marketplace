...
export default async function Collection() {
  ...
  const { data: myCards } = await supabase
    .from("user_cards")
    .select(
      "id, quantity, purchase_price, condition, collection_id, manual_price, variant, cards(name, image_small, tcgplayer_market_price, set_name, card_number, set_total, release_year, rarity, price_normal, price_holofoil, price_reverse_holofoil, price_1st_edition_holofoil)"
    )
    .eq("user_id", user!.id)
