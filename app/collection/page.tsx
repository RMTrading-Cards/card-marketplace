...
export default async function Collection() {
  ...
  const [cardsResult, sealedResult] = await Promise.all([
    supabase
      .from("user_cards")
      .select(
        "id, quantity, purchase_price, condition, collection_id, manual_price, variant, created_at, sold_price, sold_at, is_graded, grade_value, cards(id, name, image_small, tcgplayer_market_price, set_name, card_number, set_total, release_year, rarity, price_normal, price_holofoil, price_reverse_holofoil, price_1st_edition_holofoil, raw_skus, region)"
      )
      .eq("user_id", user!.id)
